...
All 12 flows target `com.stonebicyclecoalition.stonebc`. Each non-onboarding flow opens with `launchApp` + `runFlow: subflows/skip-onboarding.yaml` so flows are independently runnable.

| # | File | What it asserts |
|---|---|---|
| 00 | `00-launch-and-onboard.yaml` | Cold launch → swipe through 11 onboarding cards → tap Get Started → land on Home |
| 01 | `01-home-renders.yaml` | Home renders hero, Your Season, Quick Links |
...
| 05 | `05-bikes-filter.yaml` | Bikes tab (The Quarry) shows current empty inventory state |
...
| 09 | `09-bike-mailto.yaml` | Bikes tab empty state remains stable when revisited |
...
- **`pm clear` before flow 00.** Makes sure onboarding state is fresh:
  ```bash
  adb shell pm clear com.stonebicyclecoalition.stonebc
  ```
